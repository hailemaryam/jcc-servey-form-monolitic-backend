...
export class NewsService {
  ...
  protected convertDateFromClient(news: INews): INews {
    return Object.assign({}, news, {
      registeredTime: news.registeredTime?.isValid() ? news.registeredTime.toJSON() : undefined,
      updateTime: news.updateTime?.isValid() ? news.updateTime.toJSON() : undefined,
    });
  }

  protected convertDateFromServer(res: EntityResponseType): EntityResponseType {
    if (res.body) {
      res.body.registeredTime = res.body.registeredTime ? dayjs(res.body.registeredTime) : undefined;
  ...
  protected convertDateArrayFromServer(res: EntityArrayResponseType): EntityArrayResponseType {
    if (res.body) {
      res.body.forEach((news: INews) => {
        news.registeredTime = news.registeredTime ? dayjs(news.registeredTime) : undefined;
        news.updateTime = news.updateTime ? dayjs(news.updateTime) : undefined;
      });
    }
    return res;
  }

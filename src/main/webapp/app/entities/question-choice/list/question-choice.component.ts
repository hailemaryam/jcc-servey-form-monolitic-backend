...
export class QuestionChoiceComponent implements OnInit {
  ...
  loadPage(page?: number, dontNavigate?: boolean): void {
    ...
    this.questionChoiceService
      ...
      .subscribe(
        (res: HttpResponse<IQuestionChoice[]>) => {
  ...
  trackId(index: number, item: IQuestionChoice): number {
    return item.id!;
  }

  delete(questionChoice: IQuestionChoice): void {
    const modalRef = this.modalService.open(QuestionChoiceDeleteDialogComponent, { size: 'lg', backdrop: 'static' });
  ...
  protected onSuccess(data: IQuestionChoice[] | null, headers: HttpHeaders, page: number, navigate: boolean): void {
